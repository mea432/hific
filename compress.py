...
File = namedtuple(
    "File", ["original_path", "compressed_path", "compressed_num_bytes", "bpp"]
)
...
def prepare_dataloader(args, input_dir, output_dir, batch_size=1):
    ...
    eval_loader = datasets.get_dataloaders(
        "evaluation",
        root=input_dir,
...
def prepare_model(ckpt_path, input_dir):
    ...
    logger = utils.logger_setup(
        logpath=os.path.join(input_dir, f"logs_{time.time()}"),
        filepath=os.path.abspath(__file__),
    )
    loaded_args, model, _ = utils.load_model(
        ckpt_path,
        logger,
        device,
    ...
    with torch.no_grad():
        for idx, (data, bpp, filenames) in enumerate(tqdm(data_loader), 0):
            ...
            actual_bpp, theoretical_bpp = compression_utils.save_compressed_format(
                compressed_output, out_path=out_path
            )
            model.logger.info(
                f"Attained: {actual_bpp:.3f} bpp vs. theoretical: {theoretical_bpp:.3f} bpp."
            )
...
def compress_and_decompress(args):
    # Reproducibility
    make_deterministic()
    perceptual_loss_fn = ps.PerceptualLoss(
    ...
    device = utils.get_device()
    logger = utils.logger_setup(
        logpath=os.path.join(args.image_dir, "logs"), filepath=os.path.abspath(__file__)
    )
    loaded_args, model, _ = utils.load_model(
        args.ckpt_path,
        logger,
    ...
    dictify = lambda x: dict(
        (n, getattr(x, n)) for n in dir(x) if not (n.startswith("__") or "logger" in n)
    )
    ...
    bpp_total, q_bpp_total, LPIPS_total = (
        torch.Tensor(N),
        torch.Tensor(N),
        torch.Tensor(N),
    )
    ...
    with torch.no_grad():
        for idx, (data, bpp, filenames) in enumerate(tqdm(eval_loader), 0):
                ...
                if args.save is True:
                    ...
                    compression_utils.save_compressed_format(
                        ...
                        out_path=os.path.join(
                            args.output_dir, f"{filenames[0]}_compressed.hfc"
                        ),
                    )
            ...
            perceptual_loss = perceptual_loss_fn.forward(
                reconstruction, data, normalize=True
            )

            if args.metrics is True:
                # [0., 1.] -> [0., 255.]
                psnr = metrics.psnr(
                    reconstruction.cpu().numpy() * max_value,
                    data.cpu().numpy() * max_value,
                    max_value,
                )
            ...
            for subidx in range(reconstruction.shape[0]):
                if B > 1:
                    ...
                    q_bpp_per_im = (
                        float(q_bpp.item())
                        if type(q_bpp) == torch.Tensor
                        else float(q_bpp)
                    )

                fname = os.path.join(
                    args.output_dir,
                    "{}_RECON_{:.3f}bpp.png".format(filenames[subidx], q_bpp_per_im),
                )
                torchvision.utils.save_image(
                    reconstruction[subidx], fname, normalize=True
                )
                output_filenames_total.append(fname)

            bpp_total[n : n + B] = bpp.data
            q_bpp_total[n : n + B] = (
                q_bpp.data if type(q_bpp) == torch.Tensor else q_bpp
            )
    ...
    logger.info(
        "Complete. Reconstructions saved to {}. Output statistics saved to {}".format(
            args.output_dir, df_path
        )
    )
    delta_t = time.time() - start_time
...
def main(**kwargs):
    description = (
        "Compresses batch of images using learned model specified via -ckpt argument."
    )
    ...
    parser.add_argument(
        "-rc",
        "--reconstruct",
        help="Reconstruct input image without compression.",
        action="store_true",
    )
    parser.add_argument(
        "-save", "--save", help="Save compressed format to disk.", action="store_true"
    )
    parser.add_argument(
        "-metrics",
        "--metrics",
        help="Evaluate compression metrics.",
        action="store_true",
    )
    parser.add_argument(
        "-cpp",
        "--use_cpp",
        help="Use C++ implementation for ANS coding.",
        action="store_true",
    )
